        ...
        >
          ...
          <div style={styles.AddCommentBox}>
            <p style={styles.Reset}>
              {renderedComments.length ? 'Reply:' : 'Write a comment:'}
            </p>
            <textarea
              name="comment"
              rows={5}
...
export class Root extends React.Component<RootProps, RootState> {
  constructor(props: RootProps) {
    ...
    createHighlighter(
      props.Sandbox,
      this.onBeforeHighlight.bind(this),
      this.onNewHighlight.bind(this),
    );
  ...
  onDocumentClick(event: Event) {
    ...
    const clickNotFromHighlighting =
      Date.now() > 100 + this.state.LastSelectTime;
    ...
    if (
      clickNotFromHighlighting &&
      !highlightClicked &&
      !annotationClicked(elem, 5)
    ) {
  ...
  onNewHighlight(range: Range, highlights: Element[]) {
    ...
    this.setState({
      InProgressAnnotation: (
        <Annotation Highlight={inProgressHighlight} Comments={[]}></Annotation>
      ),
    });
  }
